...
void relay_board_detect(void)
{
    ...
    ESP_LOGI(TAG, "Board type: %s, relays: %d",
...
void relay_send_state_confirmation(int index)
{
    ...
    if (err == ESP_OK) {
        ESP_LOGD(TAG, "Sent relay state confirmation: %c%c", relay_char, state_char);
    } else {
        ESP_LOGW(TAG, "Failed to send relay state confirmation");
    }
}
...
void relay_handle_command(const char *cmd_data)
{
    ...
    // Handle relay command: "a" = toggle relay 0, "a0" = set relay 0 OFF, "a1" = set relay 0 ON
    ...
    while (1) {
        ...
        for (int i = 0; i < buttons_to_check; i++) {
            ...
            if (current_state == 1 && g_relay_button_states[i].last_state == 0) {
                // Debounce check
                if ((now_ms - g_relay_button_states[i].last_press_time) > BUTTON_DEBOUNCE_MS) {
...
void relay_init(void)
{
    ...
    if (g_board_type == BOARD_TYPE_8_RELAY) {
        ...
        io_conf.pull_up_en = GPIO_PULLUP_DISABLE;
        io_conf.pull_down_en = GPIO_PULLDOWN_DISABLE;
        io_conf.pin_bit_mask = ((1ULL << RELAY_16_PIN_DATA) | 
                                 (1ULL << RELAY_16_PIN_CLOCK) | 
                                 (1ULL << RELAY_16_PIN_LATCH) |
                                 (1ULL << RELAY_16_PIN_OE));
        gpio_config(&io_conf);
